...
try:
    ...
    def generate_multi_room_proposal(client_name, room_list, project_mode, fitout_pkgs):
        ...
        def add_body_text(text):
            ...
            p_body.paragraph_format.space_after = Pt(12) # Breathing room
            p_body.paragraph_format.line_spacing = 1.25  # More comfortable gap
            # CHANGED: Left alignment avoids wide gaps with newlines
        ...
        # --- BULLET POINT HELPER (With Fail-Safe) ---
        def add_bullet_list(items):
            for item in items:
                try:
                    # Try to use the standard bullet style
                    p = doc.add_paragraph(item, style='List Bullet')
                except:
                    # Fallback if style doesn't exist in template
                    p = doc.add_paragraph(f"• {item}")
                
                # Force Left Alignment and Spacing
                p.paragraph_format.line_spacing = 1.3
                p.paragraph_format.space_after = Pt(10)
                p.alignment = WD_ALIGN_PARAGRAPH.LEFT
                
                # Force Indentation for professional look
                ...
                for run in p.runs:
                    run.font.name = 'Helvetica'
                    run.font.size = Pt(10) # 10pt is standard for long legal lists
        ...
        for room in room_list:
            ...
            if "Fit-Out" in r_type:
                ...
                doc.add_paragraph("") # Spacer before BOM
        ...
        # --- BULLET LIST FOR EXCLUSIONS ---
        doc.add_paragraph("We exclude the following items from our installation:", style='Normal').paragraph_format.space_after = Pt(10)
        ...
        exclusions_list = [
            ...
            "All Webex/Teams/Exchange credentials must be provided prior to attending site. Admin credentials for any Webex/Teams endpoint must be provided to Alder Technology for the duration of any deployment including temporary administrator cloud tenant access and other software admin access.",
            "The network must be active and configured for Teams/Webex prior to attending site.",
            "All external services provided by the client or their nominated systems integrator (including Microsoft Teams Room/Webex accounts, Azure Intune registrations, Exchange, Skype for Business, etc.) are the responsibility of the client.",
            "Software updates and the impact on the hardware, user experience, and the operational state of the system are the sole responsibility of the client.",
            "Any updates requiring Alder Technology site attendance incur a Service call out fee and hourly rate, unless a service level agreement is in place.",
        ...
        add_bullet_list(exclusions_list)
