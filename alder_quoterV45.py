...
try:
    ...
    def load_internal_data():
        ...
        # SHARED OPTION DATA
        booking_panel = {
            "name": "Room Booking Panel",
            "desc": "Crestron TS-1070 with Lightbar kit and Multi Surface Mount",
            "price": 2200.00
        }
        ...
        audio_upgrade_98 = {
            ...
            "total_price": 15860.00, # $11,860 Hardware + $4,000 Services
            "items": [
    ...
    def generate_multi_room_proposal(client_name, room_list, project_mode, fitout_pkgs):
        ...
        def add_body_text(text):
            ...
            # CHANGED: Reduced to 6pt to tighten layout
        ...
        options_to_list = [] # List of tuples: (Room Name, Option Name, Option Price)
        ...
        for room in room_list:
            ...
            if "Fit-Out" in r_type:
                ...
                # CHECK FOR OPTIONS TO ADD TO LIST
                if 'options' in pkg:
                    for opt in pkg['options']:
                        options_to_list.append((name, opt['name'], opt['price']))
                
                if 'audio_upgrade' in pkg:
                    upg = pkg['audio_upgrade']
                    options_to_list.append((name, upg['name'], upg['total_price']))
        ...
        if options_to_list:
            ...
            opt_table.columns[0].width = Cm(5.0)
            opt_table.columns[1].width = Cm(9.0)
            opt_table.columns[2].width = Cm(4.0)

            h_row = opt_table.rows[0]
            format_row(h_row, 0.9)
            h_cells = h_row.cells
            h_cells[0].text = "Room Name"
            h_cells[1].text = "Upgrade Description"
            h_cells[2].text = "Additional Cost (Ex GST)"
            for c in h_cells:
                shade_cell(c, "E7E6E6")
                c.paragraphs[0].runs[0].bold = True

            for opt_room, opt_desc, opt_price in options_to_list:
                r_o = opt_table.add_row()
                format_row(r_o, 0.8)
                cells_o = r_o.cells
                cells_o[0].text = opt_room
                cells_o[1].text = opt_desc
                cells_o[2].text = f"${opt_price:,.2f}"
                cells_o[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
        ...
        for room in room_list:
            ...
            # --- IMAGE PLACEHOLDER ---
            # Create a separate table for image to keep spacing clean
            ...
            # REMOVED SPACER HERE to save vertical space
            
            # --- INSERT TEXT BLOCKS (Fit-Out Specific) ---
            if "Fit-Out" in r_type:
            ...
            if "Fit-Out" in r_type:
                ...
                # --- 3. OPTIONS TABLE (Attached to Room) ---
                if 'options' in pkg and pkg['options']:
                    # Separate header for options
                    r_opt_h = table_room.add_row()
                    format_row(r_opt_h, 0.8)
                    c_opt = r_opt_h.cells
                    c_opt[0].merge(c_opt[4])
                    c_opt[0].text = "3. Optional Extras (Not included in Total)"
                    shade_cell(c_opt[0], "FCE4D6")
                    c_opt[0].paragraphs[0].runs[0].bold = True
                    
                    for opt in pkg['options']:
                        add_row(opt['name'], 1, opt['desc'], opt['price'])

                # --- 4. AUDIO UPGRADE TABLE (Attached to Room) ---
                if 'audio_upgrade' in pkg:
                    upg = pkg['audio_upgrade']
                    ...
                    c_upg = r_upg_h.cells
                    c_upg[0].merge(c_upg[4])
                    c_upg[0].text = "4. Optional Upgrade: " + upg['name']
                    ...
                    for item_code, qty, desc, price in upg['items']:
                        add_row(item_code, qty, desc, price)
                    
                    # Total Row for Upgrade
                    r_upg_tot = table_room.add_row()
                    format_row(r_upg_tot, 0.9)
                    c_ut = r_upg_tot.cells
                    c_ut[0].merge(c_ut[3])
                    c_ut[0].text = "Total Upgrade Price (Ex GST):"
                    c_ut[0].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
                    c_ut[0].paragraphs[0].runs[0].bold = True
                    c_ut[4].text = f"${upg['total_price']:,.2f}"
                    c_ut[4].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
                    c_ut[4].paragraphs[0].runs[0].bold = True
