...
try:
    ...
    def load_internal_data():
        ...
        # 98" AUDIO UPGRADE DATA (Used for breakdown on Page 2, and Summary on Page 1)
        audio_upgrade_98 = {
    ...
    def generate_multi_room_proposal(client_name, room_list, project_mode, fitout_pkgs):
        ...
        for room in room_list:
            ...
            if "Fit-Out" in r_type:
                ...
                # INCREMENT COUNTERS
                qty_booking_panels += 1 # Every Fit-Out gets a panel available
        ...
        if qty_booking_panels > 0 or qty_audio_upgrades > 0:
            ...
            # 5-Column BOM Style for Consolidated View
            opt_table = doc.add_table(rows=1, cols=5)
            opt_table.style = 'Table Grid'
            opt_table.autofit = False
            opt_table.columns[0].width = Cm(4.0) # Name
            opt_table.columns[1].width = Cm(1.2) # Qty
            opt_table.columns[2].width = Cm(8.0) # Desc
            opt_table.columns[3].width = Cm(2.5) # Unit
            opt_table.columns[4].width = Cm(2.5) # Total
            ...
            if qty_audio_upgrades > 0:
                # 98" Audio Upgrade Price logic is fixed in the data structure
                # Find the data to reference price
