...
try:
    ...
    def load_internal_data():
        ...
        # ---------------------------------------------------------
        # 1. FIT-OUT PACKAGES (Your Custom Logic)
        # ---------------------------------------------------------
        # Ranges: 55(3m), 65(4.5m), 75(5.5m), 86(6.5m), 98(7.5m)
        ...
        fitout_packages = {
            ...
            "Fit-Out 86": {
                ...
                "services": 3500.00, # Installation, Staging, PM, Engineering
                "ms_price": 850.00,  # Estimated MS
                "items": []
            },
            "Fit-Out 98": {
                "max_distance": 7.5,
        ...
        # ---------------------------------------------------------
        # 2. DATA#3 PACKAGES (Placeholder Data)
        # ---------------------------------------------------------
        pricelist_data = [
            {
                "max_distance": 3.0,
                "tier_name": "Small Meeting Room",
                "cisco_items": ["Cisco Room Bar", "Cisco Navigator"],
                "display_model": "Samsung 55\" Commercial",
                "display_price": 1500.00,
                "mount_model": "Tilt Wall Mount",
                "mount_price": 150.00,
            ...
            {
                "max_distance": 5.5,
                "tier_name": "Medium Meeting Room",
                "cisco_items": ["Cisco Room Bar Pro", "Cisco Navigator"],
                "display_model": "Samsung 75\" Commercial",
                "display_price": 2800.00,
                "mount_model": "Heavy Duty Wall Mount",
                "mount_price": 200.00,
                "cables_misc": "HDMI, Patch Leads & Misc",
                "cables_price": 150.00,
            ...
            {
                "max_distance": 8.0,
                "tier_name": "Large Meeting Room",
                "cisco_items": ["Cisco Room Kit EQ", "Quad Camera", "Navigator"],
                "display_model": "Dual Samsung 75\" Commercial",
                "display_price": 5600.00,
                "mount_model": "Dual Heavy Duty Mounts",
                "mount_price": 400.00,
                "cables_misc": "Integration Kit",
                "cables_price": 300.00,
    ...
    def refresh_room_list():
        ...
        for index, room in enumerate(ADDED_ROOMS):
            ...
            if "Fit-Out" in room['type']:
                display_txt = f"📦 {room['type']} ({room['distance']}m)"
            else:
                display_txt = f"🔌 {room['type']} ({room['distance']}m)"
    ...
    def on_add_room():
        ...
        if mode == "Data#3 (Cisco)":
            ...
            room_entry['pkg_key'] = mapped_data # Store the raw Excel Key
