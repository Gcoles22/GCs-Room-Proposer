...
try:
    ...
    def load_pricelist_and_packages():
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                # Only add to pricelist_data if it's NOT a Fit-Out specific row
                # (Unless your logic requires Fit-Out rows to also be in the Data#3 list, 
                # but usually we want strict separation)
                if "Fit-Out" not in r_name:
    ...
    def generate_multi_room_proposal(client_name, room_list, project_mode, fitout_pkgs):
        ...
        for room in room_list:
            ...
            if "Fit-Out" in r_type:
                # IMPORTANT: For fit-out, r_type name must match the dictionary key
                # Since we appended "(x-ym)" in the UI, we might need to rely on the stored config or strip name
                # However, in this code version, we pass the 'pkg' data directly in room['config'] if possible
                # But to stay compatible, we look up or use config.
                
                # Check if we stored the raw package name or full data
                pkg = fitout_pkgs.get(r_type)
                
                # Fallback if UI passed a modified name, but usually we handle this in UI layer
    ...
    def update_dropdown_options(choice):
        ...
        if choice == "Data#3 (Cisco)":
            # Sort is already done in load function, but good to be safe
            ...
            if FITOUT_PACKAGES_DYN:
                for pkg_name, pkg_data in FITOUT_PACKAGES_DYN.items():
                    label = pkg_name 
                    display_options.append(label)
                    DROPDOWN_MAPPING[label] = pkg_name # Map label to key string
            else:
                display_options = ["No Fit-Out Pkgs in Excel"]
    ...
    def on_add_room():
        ...
        room_entry = {'name': r_name, 'distance': r_dist}
        ...
        if mode == "Data#3 (Cisco)":
            ...
            # In Fit-Out mode, mapped_data IS the package key name
            room_entry['type'] = selection_label
            room_entry['pkg_key'] = mapped_data 
