...
try:
    ...
    def generate_multi_room_proposal(client_name, room_list):
        ...
        hdr[2].text = "Upfront Supply & Services"
        hdr[3].text = "Managed Service Agreement (Year 1)"
        ...
        # ---------------------------------------------------------
        # PAGE 2+: DETAILED ROOM BREAKDOWNS (CONDENSED)
        # ---------------------------------------------------------
        ...
        for room in room_list:
            ...
            # Columns: Item Category | Description | Price/Note
            ...
            row_hdr[0].merge(row_hdr[1]).merge(row_hdr[2])
            ...
            # 2. SECTION: CISCO (DATA#3)
            row_sec1 = table_room.add_row().cells
            row_sec1[0].merge(row_sec1[1]).merge(row_sec1[2])
            ...
            for item in data['cisco_items']:
                ...
                r[0].text = "Video Conferencing"
                r[1].text = item
                r[2].text = "Supplied by Data#3"
                r[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT

            # 3. SECTION: ALDER SUPPLY
            row_sec2 = table_room.add_row().cells
            row_sec2[0].merge(row_sec2[1]).merge(row_sec2[2])
            row_sec2[0].text = "2. Alder Technology Supply Scope (Upfront)"
            shade_cell(row_sec2[0], "E7E6E6") # Light Grey
            row_sec2[0].paragraphs[0].runs[0].bold = True

            # Helper to add rows
            def add_spec_row(cat, desc, price_val):
                r = table_room.add_row().cells
                r[0].text = cat
                r[1].text = desc
                r[2].text = f"${price_val:,.2f}"
                r[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT

            add_spec_row("Visual Display", data['display_model'], data['display_price'])
            add_spec_row("Mounting", data['mount_model'], data['mount_price'])
            add_spec_row("Cabling", data['cables_misc'], data['cables_price'])
            add_spec_row("Services", "Staging, Installation, Commissioning & PM", data['service_price'])
            ...
            row_sub[0].merge(row_sub[1])
            row_sub[0].text = "UPFRONT SUB-TOTAL (EX GST):"
            row_sub[0].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
            row_sub[0].paragraphs[0].runs[0].bold = True
            row_sub[2].text = f"${upfront_subtotal:,.2f}"
            row_sub[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
            row_sub[2].paragraphs[0].runs[0].bold = True

            # 4. SECTION: MANAGED SERVICES
            row_sec3 = table_room.add_row().cells
            row_sec3[0].merge(row_sec3[1]).merge(row_sec3[2])
            row_sec3[0].text = "3. Managed Services"
            shade_cell(row_sec3[0], "E7E6E6") # Light Grey
            row_sec3[0].paragraphs[0].runs[0].bold = True

            add_spec_row("Support", "Annual Managed Service Agreement (Year 1)", data['ms_annual'])
        ...
        for c in headers: shade_cell(c, "D9E2F3")
