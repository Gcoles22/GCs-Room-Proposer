        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                if "Fit-Out" in r_name:
                    # Parse Extra Items String: "Name|Cost|Qty; Name|Cost|Qty"
                    extra_items_list = []
                    if r_extras_str:
                        ...
                        for g in groups:
                            parts = g.split('|')
                            if len(parts) == 3:
                                # Format: (Category, Desc, Price, Qty)
                                # We infer Category generically as "Audio/Network"
                    ...
                    fitout_packages[r_name] = {
                        ...
                        "vc": ("Video Conf", r_vc_item, 3900.00), # Assume 3900 base for bar, or you can add column M in excel for VC price
                        "cables": ("Cabling", r_cab_desc, r_cab_price),
                        "services": r_svc_price,
                        "ms_price": r_ms_price, # Directly use row price
