...
try:
    ...
    FITOUT_PACKAGES = {
        "Fit-Out 55": {
            ...
            "services": 3000.00,
            "ms_tier_match": "Medium", # Will look for MS price of 'Medium' tier in Excel
    ...
    def generate_multi_room_proposal(client_name, room_list, project_mode, pricelist_data):
        ...
        for room in room_list:
            ...
            row_hdr[0].text = f"ROOM: {name} - {r_type}"
    ...
    def refresh_room_list():
        ...
        for index, room in enumerate(ADDED_ROOMS):
            ...
            if "Fit-Out" in room['type']:
                ...
                display_txt = f"🔌 {room['name']} ({room['type']} - {room['distance']}m)"
    ...
    def update_dropdown_options(choice):
        ...
        if choice == "Data#3 (Cisco)":
            ...
            entry_distance.configure(state="normal", placeholder_text="Distance (m)")
        else:
            options = list(FITOUT_PACKAGES.keys())
            entry_distance.configure(state="disabled", placeholder_text="N/A for Fit-Out")
    ...
    def on_add_room():
        ...
        room_entry = {'name': r_name, 'type': r_type}

        # DATA#3 MODE LOGIC (Requires Distance & Excel Config)
        if mode == "Data#3 (Cisco)":
            try:
                r_dist_str = entry_distance.get().strip()
                if not r_dist_str:
                    tkinter.messagebox.showwarning("Missing Data", "Distance is required for Data#3 projects.")
                    return
            ...
            room_entry['distance'] = r_dist
            room_entry['config'] = config
        
        # FIT-OUT MODE LOGIC (Fixed Packages)
        else:
            room_entry['distance'] = 0 # Not used but good to initialize
    ...
    # Distance
    entry_distance = ctk.CTkEntry(builder_frame, placeholder_text="Distance (m) - Required for Data#3", width=350)
