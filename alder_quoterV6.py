...
try:
    ...
    def generate_multi_room_proposal(client_name, room_list):
        ...
        # --- TEMPLATE LOADING ---
        template_name = "Alder_Template.docx"
        ...
        doc.add_paragraph(
            ...
            "1. A Master Financial Schedule summarizing all rooms (Upfront Hardware + Year 1 Services).\n"
        ...
        for room in room_list:
            ...
            room_total = hw_cost + svc_cost + ms_annual
            ...
            row[3].text = f"${(svc_cost + ms_annual):,.0f}"
            row[4].text = f"${room_total:,.2f}"

        doc.add_paragraph("\n")
        p_total = doc.add_paragraph()
        p_total.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        runner = p_total.add_run(f"TOTAL UPFRONT PROJECT VALUE (EX GST): ${grand_total_project:,.2f}")
        runner.bold = True
        ...
        for room in room_list:
            ...
            # Rows
            table_a.add_row().cells[0].text, table_a.rows[-1].cells[1].text, table_a.rows[-1].cells[2].text = "Visual Display", data['display_model'], f"${data['display_price']:,.2f}"
            table_a.add_row().cells[0].text, table_a.rows[-1].cells[1].text, table_a.rows[-1].cells[2].text = "Mounting Hardware", data['mount_model'], f"${data['mount_price']:,.2f}"
            table_a.add_row().cells[0].text, table_a.rows[-1].cells[1].text, table_a.rows[-1].cells[2].text = "Cabling & Consumables", data['cables_misc'], f"${data['cables_price']:,.2f}"
            table_a.add_row().cells[0].text, table_a.rows[-1].cells[1].text, table_a.rows[-1].cells[2].text = "Professional Services", "Includes: Offsite Staging, Installation, Room Coordination, Commissioning, PM & Documentation.", f"${data['service_price']:,.2f}"
            table_a.add_row().cells[0].text, table_a.rows[-1].cells[1].text, table_a.rows[-1].cells[2].text = "Managed Service", "Year 1 (Annual Billing) - 5 Year Term Agreement", f"${data['ms_annual']:,.2f}"

            this_room_total = data['display_price'] + data['mount_price'] + data['cables_price'] + data['service_price'] + data['ms_annual']
        ...
        # --- FURTHER OPTIONS ---
        run = doc.add_heading('Further Options', level=2).runs[0]
        run.bold = True
        ...
        doc.add_heading('Managed Service Agreement', level=2)
        doc.add_paragraph(
            "An example costing is provided below. Please note the below does not constitute a quote until the scope of the MSA is finalised. "
    ...
    lbl_status = ctk.CTkLabel(app, text="Template Check: Ready...", font=("Roboto", 12))
    
    if os.path.exists("Alder_Template.docx"):
        lbl_status.configure(text="Ready (Template Found)", text_color="#00FF00")
    else:
        lbl_status.configure(text="Ready (No Template Found - Using Blank)", text_color="yellow")
